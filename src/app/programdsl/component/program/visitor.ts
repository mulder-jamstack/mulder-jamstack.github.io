...
function calcRoot(node : Parser.Root, context : Context) : object[] {
    ...
    for (const [day, schedule] of fullSchedule) {
        ...
        let roomsSchedule : ((TimeSlot | null)[] | undefined)[] = []
            ...
            for (let roomID = 0; roomID < roomsSchedule.length; roomID++) {
                ...
                if(roomSchedule){
                    ...
                    if(slot && slot.getEvent()){
                        ...
                        if(event!.abstract) theEvent["description"] = event!.abstract
...
class Schedule {
    ...
    add(room:Room, timeslot : TimeSlot){
        let slots : (TimeSlot | null)[] | undefined = this.schedulePerRoom.get(room);
        if(!slots){
            slots = []
    ...
    sortAndFillHoles(){
        ...
        for (let [room, slots] of this.schedulePerRoom) {
            let sorted = slots.sort((obj1 :TimeSlot|null, obj2 :TimeSlot|null) => { return obj1!.compareTo(obj2)});
